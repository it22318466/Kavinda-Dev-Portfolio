...
const Skills = () => {
  ...
  const skillCategories = {
    ...
    "Tools & Others": [
      ...
      skills.find((s) => s.name === "Bootstrapping"),
      skills.find((s) => s.name === "Problem Solving"),
      skills.find((s) => s.name === "Postman"),
      skills.find((s) => s.name === "Laravel"),
      skills.find((s) => s.name === "Canva"),
      skills.find((s) => s.name === "Picsart"),
      skills.find((s) => s.name === "Git Lab"),
      skills.find((s) => s.name === "Bitbucket"),
      skills.find((s) => s.name === "FastAPIs"),
            ...
            ([category, categorySkills], categoryIndex) => (
              <FadeIn key={category} delay={categoryIndex * 100}>
                <div className="relative bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-primary/30 transition-all duration-300 group">
                    ...
                    {categorySkills.map((skill, skillIndex) => {
                      ...
                      return (
                        <div key={skill.id} className="space-y-2">
                          <div className="flex items-center justify-between hover:bg-primary/10 hover:border-primary/20 transition-all duration-300 hover:scale-110">
                  ...
                  {/* Hover Glow Effect */}
                  <div className="absolute inset-0 bg-linear-to-br from-primary/0 to-primary/5 group-hover:from-primary/5 group-hover:to-primary/5 group-hover:to-primary/5 rounded-2xl transition-all duration-300 pointer-events-none"></div>
                </div>
              </FadeIn>
            ),
          )}
